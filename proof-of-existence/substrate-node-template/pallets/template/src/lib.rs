...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		/// parameters. [something, who]
		SomethingHappened(),
		/// Event emmited when a proof has been claimed. [who, claim]
		ClaimCreated(T::AccountId, BoundedVec<u8, T::MaxBytesInHash>),
		/// Event emmited when a claim is revoked byt the owner. [who, claim]
	...
	// These functions materialize as extrinsics, which are often comapred to transations.
	// Dispatchable functions must be annotated with a weight and must return a DispatchResult.
	#[pallet::call]
	impl<T: Config> Pallet<T> {
		#[pallet::weight(1_000)]
